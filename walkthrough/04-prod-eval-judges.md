...
| `JUDGE_API_KEY is missing` / env cell prints `JUDGE_MODEL=None` | The Secret exists but the Notebook CR does not inject it. Apply **both** `secret-wings3-judge-llm.yaml` **and** `workbench-wings3-demo.yaml` (explicit `secretKeyRef`, not `envFrom` — RHOAI start/stop drops `envFrom`). Then **start** the workbench. Do not commit the token. |
